...
export const useMessageStore = defineStore('message', {
  ...
  actions: {
    ...
    updateContactUnread(contactId, unreadCount) {
      console.log(`更新联系人${contactId}的未读数: ${unreadCount}`)
      this.contactUnreadMap[contactId] = unreadCount
      this.recalculateTotal()
      this.updateTabBarBadge()
      console.log('更新后contactUnreadMap:', this.contactUnreadMap)
    },

    // 增加未读消息数
    incrementUnread(contactId, count = 1) {
      console.log(`incrementUnread调用: contactId=${contactId}, count=${count}`)
      console.log('调用栈:', new Error().stack)
      
      // 验证contactId是否合理
      if (!contactId || contactId <= 0) {
        console.log('无效的contactId:', contactId)
        return
      }
      
      const currentCount = this.contactUnreadMap[contactId] || 0
      const newCount = currentCount + count
      console.log(`更新联系人${contactId}未读数: ${currentCount} -> ${newCount}`)
    ...
    recalculateTotal() {
      ...
      console.log('重新计算总未读数:', {
        contactUnreadMap: this.contactUnreadMap,
        contactUnreadSum: contactUnreadSum,
