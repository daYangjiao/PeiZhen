...
- 前端页面清单已对齐当前 `frontend/mini-program/pages.json` 和 `frontend/admin/src/router/index.js`，包含聊天单条回复、时长费用重提和资质预览口径。
- 数据库清单已覆盖 `database/student.sql` 基线、历史 `database/*.sql` 增量和当前 `db/*.sql` 自动部署增量，最新增量为聊天回复引用字段。
