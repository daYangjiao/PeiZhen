...
| `pages/role-escort/profile` | 我的 | 陪诊师个人中心，展示资料、收入、资质门禁状态、服务统计入口和按真实评价数展示的好评率；无评价显示“暂无评价”；不因资质未通过自动弹窗，资质入口用状态标签提示。 | `/attendant/profile/{...}` |
...
| `subpkg/order/order-detail` | 订单详情 | 用户订单详情，查看订单、支付尾款、取消、申诉、确认时长费用、补差额支付、查看二维码/联系陪诊师；待确认时长时展示陪诊师提交说明；陪诊师星级展示平均评分，按 `attendantScore + attendantEvaluationCount` 判断，无评价显示“暂无评分”。 | `/api/orders/{...}/cancel?reason={...}`<br>`/api/orders/{...}/dispute-time-fee{...}`<br>`/api/orders/{...}/confirm-time-fee`<br>`/api/orders/{...}/pay-balance`<br>`/api/orders/{...}/evaluation`<br>`/ai/guide/orders/{...}/complete-info` |
...
| `subpkg/order/submit-time-fee` | 提交时长与费用 | 陪诊师提交实际服务时长、费用和说明，驱动用户确认或争议流程。 | `/attendant/orders/{...}`<br>`/attendant/orders/{...}/end?actualDuration={...}&attendantTimeRemark={...}` |
...
| `subpkg/appointment-flow/02-appointment-form` | 服务预约 | 普通预约表单，填写患者/医院/时间/需求，创建预约并生成公共待接单订单；普通下单不自动指定陪诊师，支付后进入接单大厅。联系信息输入使用统一清洗和回写逻辑，兼容 H5/App/小程序；服务时段按钮采用上下结构，跨日结束时间不挤出按钮。 | `/ai/guide/appointments`<br>`/ai/guide/orders`<br>`/api/users/current` |
