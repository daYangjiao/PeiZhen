  ...
  <view class="container">
    ...
    <view class="bottom-nav">
      <view class="bottom-left">
        ...
        >
          <image src="/static/emergency.png" class="nav-icon"></image>
          <text>取消</text>
        </view>
      </view>
...
const handleBalancePayResult = async (isPaid) => {
  ...
  if (isPaid) {
    ...
    uni.navigateTo({
      url: `/pages/AItriage/PaymentFailedPage?orderNo=${encodeURIComponent(order.value.orderNo)}&scene=balance`
    });
  }
};
...
const handlePrimaryAction = () => {
  ...
  if (status === 6) {
    if (!hasEvaluated.value) {
      if (orderNo) {
        uni.navigateTo({
          url: `/pages/Evaluate/Evaluate?orderNo=${encodeURIComponent(orderNo)}`
        });
      }
    } else {
      uni.navigateTo({
