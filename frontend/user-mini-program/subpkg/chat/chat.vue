...
const headerSubtitle = computed(() => '陪诊师 · 在线沟通中')
...
const chooseImage = (sourceType) => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      // 先上传
      try {
        const uploadRes = await upload('/common/upload', path)
...
const getAvatar = (msg) => {
    ...
    if (msg.senderAvatar && msg.senderAvatar !== '/static/user-placeholder.png' && msg.senderAvatar !== '/static/doctor-avatar.png') {
        console.log('使用消息中的发送者头像:', msg.senderAvatar);
    ...
    if (targetAvatar.value && targetAvatar.value !== '/static/user-placeholder.png' && targetAvatar.value !== '/static/doctor-avatar.png') {
        console.log('使用预加载目标头像:', targetAvatar.value);
        return targetAvatar.value;
    }
    
    // 默认使用用户占位头像（与陪诊师端保持一致）
...
$primary-color: #2979ff; $bg-color: #F5F7FA; $text-main: #333; $bubble-other: #FFF; $bubble-self: $primary-color;
...
.input {
    ...
    padding: 0 20rpx; font-size: 30rpx; box-sizing: border-box;
}
...
.send-btn {
    background: #07C160; color: #fff; padding: 10rpx 20rpx; border-radius: 8rpx; font-size: 26rpx;
}

/* 面板区域 */
.panel-area {
    height: 500rpx; background: #F7F7F7; border-top: 1rpx solid #E5E5E5;
    overflow: hidden;
}
...
.more-icon-box {
    width: 110rpx; height: 110rpx; background: #FFFFFF; border-radius: 24rpx;
