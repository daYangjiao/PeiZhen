...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const emojiList = ["😀", "😁", "😂", "🤣", "😃", "😄", "😅", "😆", "😉", "😊", "😋", "😎", "😍", "😘", "🥰", "😗", "😙", "😚", "🙂", "🤗", "🤩", "🤔", "🤨", "😐", "😑", "😶", "🙄", "😏", "😣", "😥", "😮", "🤐", "😯", "😪", "😫", "😴", "😌", "😛", "😜", "😝", "🤤", "😒", "😓", "😔", "😕", "🙃", "🤑", "😲", "☹️", "🙁", "😖", "😞", "😟", "😤", "😢", "😭", "😦", "😧", "😨", "😩", "🤯", "😬", "😰", "😱", "🥵", "🥶", "😳", "🤪", "😵", "😡", "😠", "🤬", "😷", "🤒", "🤕", "🤢", "🤮", "🤧", "😇", "🤠", "🤡", "🥳", "🥴", "🥺", "🤥", "🤫", "🤭", "🧐", "🤓", "😈", "👿"];
    const headerSubtitle = common_vendor.computed(() => "陪诊师 · 在线沟通中");
    ...
    const handleNewMessage = (msg) => {
      common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:272", "处理新消息:", msg);
      const msgKey = `${msg.senderId}-${msg.receiverId}-${msg.createTime}-${msg.content}`;
      if (processedMessages.has(msgKey)) {
        common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:277", "消息已处理，跳过:", msgKey);
        return;
      }
      ...
      if (isNormalChatMsg && (msg.senderId == targetUserId.value || msg.receiverId == targetUserId.value)) {
        if (msg.senderId == targetUserId.value) {
          ...
          if (!newMsg.senderAvatar || newMsg.senderAvatar === "/static/user-placeholder.png") {
            ...
            common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:297", "设置发送者头像:", newMsg.senderAvatar);
          }
      ...
      if ((msg.type === "READ_RECEIPT" || msg.content === "READ_RECEIPT") && msg.msgType == 3) {
        common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:317", "收到已读回执，更新消息状态");
        const updatedMessages = messages.value.map((m) => {
          if (m.senderId == currentUserId.value && m.isRead !== 1) {
            common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:321", "标记消息为已读:", m.id);
            return { ...m, isRead: 1 };
          }
          return m;
        });
        ...
        common_vendor.nextTick$1(() => {
          common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:332", "已读状态更新完成");
        });
        return;
      }
      if (msg.type === "MESSAGE_STATUS_UPDATE") {
        common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:339", "收到消息状态更新:", msg);
      ...
      common_vendor.index.chooseImage({
        ...
        success: async (res) => {
          ...
          try {
            ...
            common_vendor.index.__f__("error", "at subpkg/chat/chat.vue:389", e);
          }
        }
      });
    };
    const sendVoice = async (path) => {
      try {
        ...
        common_vendor.index.__f__("error", "at subpkg/chat/chat.vue:400", e);
      }
    };
    ...
    const getAvatar = (msg) => {
      ...
      if (msg.senderId === currentUserId.value) {
        ...
        common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:535", "当前用户头像:", avatarUrl);
      ...
      if (msg.senderAvatar && msg.senderAvatar !== "/static/user-placeholder.png" && msg.senderAvatar !== "/static/doctor-avatar.png") {
        common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:541", "使用消息中的发送者头像:", msg.senderAvatar);
      ...
      if (targetAvatar.value && targetAvatar.value !== "/static/user-placeholder.png" && targetAvatar.value !== "/static/doctor-avatar.png") {
        common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:547", "使用预加载目标头像:", targetAvatar.value);
        return targetAvatar.value;
      }
      common_vendor.index.__f__("log", "at subpkg/chat/chat.vue:552", "使用默认用户头像");
