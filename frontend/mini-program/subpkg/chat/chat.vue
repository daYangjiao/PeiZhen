...
const sendMessage = async (content, type) => {
  ...
  try {
    const payload = { receiverId: targetUserId.value, content, msgType: type }
    if (activeReply?.id) {
...
.message-item.self .reply-source {
...
.message-item.self .reply-source-name {
...
.message-item.self .reply-source-text {
...
.reply-preview-copy {
  ...
  min-width: 0;
  padding: 12rpx 16rpx;
  border-left: 5rpx solid var(--chat-primary);
  border-radius: 14rpx;
  background: #f4f7fb;
  display: flex;
  flex-direction: column;
