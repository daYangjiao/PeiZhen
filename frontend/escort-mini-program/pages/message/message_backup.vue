<template>
  <view class="container">
    <!-- 调试信息 -->
    <view style="padding: 20rpx; background: #f0f0f0; margin-bottom: 20rpx;">
      <text style="font-size: 24rpx; color: #666;">调试信息：</text>
      <text style="font-size: 24rpx; color: #333; margin-left: 10rpx;">总未读: {{ messageStore.totalUnreadCount }}</text>
      <text style="font-size: 24rpx; color: #333; margin-left: 10rpx;">系统未读: {{ systemUnreadCount }}</text>
      <button size="mini" @click="testLoad" style="margin-left: 10rpx;">刷新测试</button>
    </view>
...
const loadContacts = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      console.error('API返回错误码:', res.code, res.message)
    }
  } catch (e) {
