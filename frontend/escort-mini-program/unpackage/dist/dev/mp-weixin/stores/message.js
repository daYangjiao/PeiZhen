...
const useMessageStore = common_vendor.defineStore("message", {
  ...
  actions: {
    ...
    updateContactUnread(contactId, unreadCount) {
      common_vendor.index.__f__("log", "at stores/message.js:87", `更新联系人${contactId}的未读数: ${unreadCount}`);
      this.contactUnreadMap[contactId] = unreadCount;
      this.recalculateTotal();
      this.updateTabBarBadge();
      common_vendor.index.__f__("log", "at stores/message.js:91", "更新后contactUnreadMap:", this.contactUnreadMap);
    },
    // 增加未读消息数
    incrementUnread(contactId, count = 1) {
      common_vendor.index.__f__("log", "at stores/message.js:96", `incrementUnread调用: contactId=${contactId}, count=${count}`);
      common_vendor.index.__f__("log", "at stores/message.js:97", "调用栈:", new Error().stack);
      if (!contactId || contactId <= 0) {
        common_vendor.index.__f__("log", "at stores/message.js:101", "无效的contactId:", contactId);
        return;
      }
      const currentCount = this.contactUnreadMap[contactId] || 0;
      const newCount = currentCount + count;
      common_vendor.index.__f__("log", "at stores/message.js:107", `更新联系人${contactId}未读数: ${currentCount} -> ${newCount}`);
    ...
    recalculateTotal() {
      ...
      common_vendor.index.__f__("log", "at stores/message.js:144", "重新计算总未读数:", {
    ...
    restoreFromStorage() {
      try {
        ...
        common_vendor.index.__f__("error", "at stores/message.js:199", "恢复消息状态失败:", error);
      }
    },
    // 持久化存储状态
    saveToStorage() {
      try {
        ...
        common_vendor.index.__f__("error", "at stores/message.js:213", "保存消息状态失败:", error);
      }
    }
  }
});
